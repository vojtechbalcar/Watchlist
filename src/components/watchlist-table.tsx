...
export function WatchlistTable({ holdings, benchmarkLabel, compact = false }: {
  ...
  function changeSort(key: Sort) { setSort(key); setAscending(sort === key ? !ascending : true); }
  return <section aria-label="Your watchlist">
    ...
    <div className="stock-toolbar">
      <div className="stock-tabs" role="tablist" aria-label="Filter stocks">
        {["All stocks", "Ahead", "Behind"].map((tab, i) => <button key={tab} role="tab" aria-selected={filter === tab} onClick={() => setFilter(tab)}>{tab}<span>{[live.length, ahead, live.length - ahead][i]}</span></button>)}
      </div>
