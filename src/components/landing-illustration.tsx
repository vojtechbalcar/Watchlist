...
export function LandingIllustration() {
  return (
    <svg className={styles.illustration} viewBox="34 138 584 444" role="img" aria-labelledby="landing-art-title landing-art-description">
      ...
      <desc id="landing-art-description">An isometric comparison board with three raised stock tiles. The selected stock is marked in maroon. An illustrative comparison shows the stock up {landingComparison.stockReturn} percent and the benchmark up {landingComparison.benchmarkReturn} percent, leaving the stock {Math.abs(landingComparison.gapPp)} percentage points behind.</desc>
      ...
      <g transform={plane}>
        ...
        <text x="42" y="286" fill="#9ac9b0" fontSize="33" fontWeight="500" letterSpacing="-1.3">{formatPct(landingComparison.stockReturn, 1)}</text>
        <text x="221" y="286" fill="#9ac9b0" fontSize="33" fontWeight="500" letterSpacing="-1.3">{formatPct(landingComparison.benchmarkReturn, 1)}</text>
        <path d="M45 304h307" stroke="#56595f" />
        <text x="45" y="324" fill="#d4d7db" fontSize="10">Relative performance</text>
        <text x="352" y="324" textAnchor="end" fill="#efb3b4" fontSize="14" fontWeight="500">−{Math.abs(landingComparison.gapPp).toFixed(1)} pp</text>
