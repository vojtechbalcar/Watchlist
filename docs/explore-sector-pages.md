...
- Both views now share a period selector and show the stock and sector benchmark return for that period; see [[explore-period-comparisons]].
- Cards pair the logo and company identity, emphasize the price, and separate the daily move from the selected-period sector benchmark comparison. A centered bar visualizes the benchmark gap on a shared scale for the period (±3 pp for 1D, ±5 for 1W, ±10 for 1M, ±30 for YTD, ±50 for 1Y), capped at the ends. The exact number is always visible; the endpoints label the scale.
